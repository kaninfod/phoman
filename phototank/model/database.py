...
class Database(object):
    class __singleton:
        ...
        def locate_photo(self, field, value):
        ...
        def get_photos_in_album(self, album, skip=0, limit=0):
            if album.selected or album.tags_exclude or album.tags_include or album.startdate or album.enddate:
                query_string = {
                    '$or': [
                        {'_id': {'$in': album.selected}},
            ...
            self.albums.update({"_id": album.id}, {"$set": {"photo_count": album._image_count}}, upsert=False)
            return cursor
